...
class MobileDispNetS(nn.Module):
    ...
    def __init__(self, batchNorm=False, lastRelu=True, resBlock=True, maxdisp=-1, input_channel=3, get_features = False, input_img_shape=None):
        super(MobileDispNetS, self).__init__()
    ...
    def forward(self, input):
        ...
        conv1 = self.conv1(input) # 6-64
        conv2 = self.conv2(conv1) # 64-128
        conv3 = self.conv3(conv2) # 128-256

        conv4a = self.conv4(conv3) # 256-256
        ...
        concat3 = torch.cat((upconv3, upflow4, conv3), 1)
        iconv3 = self.iconv3(concat3) # 64+1+256-64
        ...
        concat0 = torch.cat((upconv0, upflow1, input), 1)
        iconv0 = self.iconv0(concat0) # 16+1+6-16
