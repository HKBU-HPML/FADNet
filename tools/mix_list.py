...
f = open("RB_FlyingThings3D_release_TRAIN.list")
flying_train = f.readlines()
f.close()

f = open("RB_FlyingThings3D_release_TEST.list")
...
for idx in rand_idx:
    ...
    train_list.append(flying_train[idx].replace("clean_disparity", "sgm_disp"))
    train_list.append(flying_train[idx].replace("clean_disparity", "sgm_disp"))
    train_list.append(flying_train[idx].replace("clean_disparity", "sgm_disp"))
...
for idx in rand_idx:
    ...
    test_list.append(flying_test[idx].replace("clean_disparity", "sgm_disp"))
