...
def disp2norm(disp, coord, focus_length):
    ...
    nx = dx / (torch.abs(torch_disp[:, :, 1:-1] - coord[0][:, :, 1:-1] * dx) + 1e-8)
    ny = dy / (torch.abs(torch_disp[:, 1:-1, :] - coord[1][:, 1:-1, :] * dy) + 1e-8)
...
def generate_normals(opt):
    ...
    for filename in files:
        ...
        #norm[norm < 0] = 0.0
        #norm[norm > 1] = 1.0
        #print("min-max:", np.min(norm), np.max(norm))
        #skimage.io.imsave(out_file.split("/")[-1].replace("exr", "png"),(norm*255).astype('uint16'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--focus_length', type=float, help='camera focus length', default=1050)
