...
class MonodepthLoss(nn.modules.Module):
    def __init__(self, n=4, downscale=1, SSIM_w=0.85, disp_gradient_w=1.0, lr_w=1.0,
        ...
        self.multiScales = [nn.AvgPool2d(self.downscale*(2**i), self.downscale*(2**i)) for i in range(self.n)]
    ...
    def forward(self, input, target):
        """
        Args:
            input [disp1, disp2, disp3, ..., dispN] (different scale)
            target [left, right]
        ...
        disp_left_est = input
        # disp_left_est = [disp_left_est[i] / 2 ** i for i in range(self.n)]
